...
public class GameController implements ActionListener, Serializable{
	...
	transient private GameView gameView;   
    private GameModel gameModel;
    private LinkedStack<GameModel> data;
    private LinkedStack<GameModel> dataRedo;
    boolean first = true;
    ...
    public GameController(int size) {
        ...
        data = new LinkedStack<GameModel>();
        dataRedo = new LinkedStack<GameModel>();       

    }
    
    public GameController(GameController source){
    	
    	this.gameModel = source.gameModel;
    	this.gameView = source.gameView;
	...
	public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() instanceof DotButton) {        
        	...
        	if (gameModel.getCurrentStatus(clicked.getColumn(),clicked.getRow()) ==
        		...
        		GameModel cloneModel = null;
     			
     			try {
     				
     				cloneModel = (GameModel) gameModel.clone(this.gameModel);     			
                    gameModel.select(clicked.getColumn(),clicked.getRow());                   
                    data.push(cloneModel);   
                    gameView.swithOnOff();   				
     				
     			} catch (CloneNotSupportedException e1) {				
     				System.out.println("whateever");
     				}
     			
				try {
					oneStep();
				} catch (LinkedQueueException e1) {
					System.out.println("whateever");
				} 
     			}
            ...
            if (clicked.getText().equals("Quit")) {        	
				...
				try {
					g = (GameModel)data.pop();
					//dataRedo.push(g);
					
	            	try {
	     				
	     				cloneModel = (GameModel) gameModel.clone(this.gameModel);
	     				dataRedo.push(cloneModel);
	            	...
	            	this.gameModel = g;    	    				
	            	this.gameView.setModel(gameModel);            	
            	...
            	GameModel redo;
				try {
					redo = (GameModel)dataRedo.pop();            		
                	try {
                		
						data.push((GameModel) redo.clone(redo));
					}catch (CloneNotSupportedException e1) {
     ...
     */
    private void oneStep() throws LinkedQueueException{
    ...
    private Point findDirection() throws LinkedQueueException{
   ...
   /**
    ...
    public boolean stackConditionRedo(){
    	
    	return dataRedo.isEmpty();
    	
    }
    
    public boolean stackConditionData(){
